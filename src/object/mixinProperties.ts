 ...
 */
 ...
 export function mixinProperties(target:any, source:any,  options?:MixinPropertiesOptions) {
    ...
    // 复制原型链上的数据
    if (source.prototype) {
        mixinProperties(target, source.prototype, { excludes, conflict,injectStatic })
    } else if (source.__proto__ && source.__proto__ != (Function as any).__proto__ && source.__proto__ != Object.prototype) {
        mixinProperties(target, source.__proto__, { excludes, conflict,injectStatic})
    }
}
